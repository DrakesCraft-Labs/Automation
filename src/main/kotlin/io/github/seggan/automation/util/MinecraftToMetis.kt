...
fun Entity.metisProperties(getInventory: Boolean = true): Value = buildTable { table ->
    ...
    if (this is LivingEntity) {
        ...
        table["maxHealth"] = getAttribute(Attribute.MAX_HEALTH)!!.value.metisValue()
    ...
    if (this is Villager) {
        table["profession"] = profession.key.key.metisValue()
...
fun ItemStack.metisProperties(): Value = buildTable { table ->
    ...
    if (meta is BannerMeta) {
        table["patterns"] = meta.patterns.map { pattern ->
            buildTable { patternTable ->
                ...
                patternTable["pattern"] = pattern.pattern.key.key.metisValue()
            }
        }.metisValue()
    }
    if (meta is BookMeta) {
    ...
    if (meta is PotionMeta) {
        // En 1.21 desaparecio PotionData, que llevaba el tipo y dos banderas aparte. Ahora la
        // duracion y la potencia van dentro del propio tipo: lo que antes era
        // (SWIFTNESS, extended) hoy es LONG_SWIFTNESS, y (SWIFTNESS, upgraded) es
        // STRONG_SWIFTNESS.
        //
        // Se mantienen igualmente las tres claves que ya veian los scripts de los jugadores --
        // type, isExtended e isUpgraded -- para no romper los que haya escritos; las banderas se
        // deducen del prefijo del tipo, y "type" se publica sin ese prefijo para que siga
        // valiendo lo que un script comparase contra "swiftness".
        val tipoPocion = meta.basePotionType
        val clavePocion = tipoPocion?.key?.key ?: "uncraftable"
        val alargada = clavePocion.startsWith("long_")
        val reforzada = clavePocion.startsWith("strong_")
        table["basePotionData"] = buildTable { basePotionDataTable ->
            basePotionDataTable["type"] = clavePocion
                .removePrefix("long_").removePrefix("strong_").metisValue()
            basePotionDataTable["isExtended"] = alargada.metisValue()
            basePotionDataTable["isUpgraded"] = reforzada.metisValue()
        }
